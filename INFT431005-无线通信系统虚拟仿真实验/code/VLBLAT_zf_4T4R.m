...
for index=1:length(EsN0)
    ...
    [temp,ber23(index)] = biterr(x(3,:),s2(3,:),log2(M)); %非理想干扰消除时第 3 层的系统误码
    [temp,ber24(index)] = biterr(x(4,:),s2(4,:),log2(M)); %非理想干扰消除时第 4 层的系统误码
    
    [temp,ber31(index)] = biterr(x(1,:),s3(1,:),log2(M)); %理想干扰消除时第1层的系统误码
    [temp,ber32(index)] = biterr(x(2,:),s3(2,:),log2(M)); %理想干扰消除时第2层的系统误码
    [temp,ber33(index)] = biterr(x(3,:),s3(3,:),log2(M)); %理想干扰消除时第 3 层的系统误码
    [temp,ber34(index)] = biterr(x(4,:),s3(4,:),log2(M)); %理想干扰消除时第 4 层的系统误码
...
grid on
figure
semilogy(EsN0,ber31,'-ko',EsN0,ber32,'-r*',EsN0,ber33,'-g.',EsN0,ber34,'-b+')
title('4R4TV-BLAST结构ZF-SIC检测算法性能，理想干扰消除')
legend('第1层','第2层','第3层','第4层')
xlabel('信噪比Es/N0')
ylabel('误比特率（BER）')
 grid on
figure
semilogy(EsN0,ber21,'-ko',EsN0,ber22,'-r*',EsN0,ber23,'-g.',EsN0,ber24,'-b+')

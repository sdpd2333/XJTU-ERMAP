function deltaf=my_rx_func(rxdata)
...
axis equal;
% subplot(2,2,1);
plot(real(rx_sig_filter),imag(rx_sig_filter),'b.');
grid on;
hold on;
%% normalization
...
rx_sig_down = rx_sig_norm.';
figure(2);clf;
axis equal;
% subplot(2,2,1);
plot(real(rx_sig_down),imag(rx_sig_down),'b.');
axis([-1.5 1.5 -1.5 1.5]);
grid on;
hold on;
%% package search
[rx_frame,cor_abs,th_max,index_s]=rx_package_search(rx_sig_down,local_sync,639,4);%639 
figure(3);clf;
axis equal;
% subplot(2,2,1);
plot(real(rx_frame),imag(rx_frame),'b.');
axis([-1.5 1.5 -1.5 1.5]);
grid on;
hold on;
...
fine_sync_seq=rx_frame(1:120);
[deltaf,out_signal3]=rx_freq_sync(rx_frame,fine_sync_seq);

figure(4);clf;
axis equal;
% subplot(2,2,2);
plot(real(out_signal3),imag(out_signal3),'b.');
axis([-1.5 1.5 -1.5 1.5]);
grid on;
hold on;
...
axis equal;
% subplot(121);
plot(real(out_signal4),imag(out_signal4),'b.');
axis([-1.5 1.5 -1.5 1.5]);
grid on;
hold on;
%% phase track
...
out_signal7=out_signal4(127+1:end);

% out_signal7=out_signal4;
%% time domain equalize
out_signal8=rx_time_equalize(out_signal7);
%% signal demod
[soft_bits_out,evm] = rx_qpsk_demod(out_signal8);
Si=[1 1 0 1 1 0 0];
m=0;
...
soft_bits_in=str_to_bits(msgStr);
tmp = xor(soft_bits_in, soft_bits_out);
error = sum(tmp)/1024;
%% crc32 check
...
% text(0.15,0.6,['调制模式：', 'QPSK']);
% text(0.15,0.4,['数据长度：', '124','bytes']);
